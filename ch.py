...
class Room:
  ...
  def _feed(self, data):
    ...
    while self._rbuf.find(b"\x00") != -1:
      ...
      for food in data[:-1]:
        if self.unicodeCompat:
          self._process(food.decode("utf8", "strict").rstrip("\r\n"))  #Encoding with UTF-8 will prevent the script from crashing when connecting to a room
        else:
          self._process(food.decode(errors="replace").rstrip("\r\n"))
      self._rbuf = data[-1]
  ...
  def _sendCommand(self, *args):
    ...
    if self._firstCommand:
      ...
      terminator = b"\r\n\x00"
    self._write(":".join(args).encode("utf8", "safely") + terminator) #Fixed issue trying to send special characters such as umlauts.
